...
local function getBestAimbotTarget(myHRP)
    if _aimbotLockedTarget and isTargetValid(_aimbotLockedTarget) then
        return _aimbotLockedTarget:FindFirstChild("HumanoidRootPart"), _aimbotLockedTarget
    end

    local shortestDist, bestHRP, bestChar = math.huge, nil, nil
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= lp and isTargetValid(plr.Character) then
            local hrp = plr.Character.HumanoidRootPart
            if hrp and hrp.Parent == plr.Character then
...
local function equipBat()
    ...
    if not currentTool or not currentTool.Name:lower():find("bat") then
        local bat = nil
        ...
        currentBat = currentTool
    end
end

local function unequipBat()
    if currentBat and currentBat.Parent == lp.Character then
        currentBat.Parent = backpack
        currentBat = nil
    end
end

local function maintainConstantSpeed(hrp, direction)
    if not hrp or not hrp.Parent then return end
    
    local targetVelocity = direction.Unit * BatAimbot.SPEED
    ...
    task.spawn(function()
        ...
        if hrp and hrp.Parent then
            ...
            if currentVel.Magnitude < BatAimbot.SPEED - 0.1 or currentVel.Magnitude > BatAimbot.SPEED + 0.1 then
                hrp.AssemblyLinearVelocity = targetVelocity
            end
        end
    end)
...
local function startBatAimbotInternal()
    if BatAimbot.batAimbotEnabled then return end
    BatAimbot.batAimbotEnabled = true
    ...
    local oldAtt = hrp:FindFirstChild("AimbotAttachment")
    local oldAlign = hrp:FindFirstChild("AimbotAlign")
    ...
    align.MaxTorque = 1000
    align.Responsiveness = 50
    align.RigidityEnabled = true

    BatAimbot.aimbotConn = RunService.Heartbeat:Connect(function()
        if not BatAimbot.batAimbotEnabled then return end
        ...
        local align2 = hrp2:FindFirstChild("AimbotAlign")
        if not align2 then
            local att2 = hrp2:FindFirstChild("AimbotAttachment")
            if not att2 then
                att2 = Instance.new("Attachment", hrp2)
                att2.Name = "AimbotAttachment"
            end
            align2 = Instance.new("AlignOrientation", hrp2)
            align2.Name = "AimbotAlign"
            align2.Attachment0 = att2
            align2.Mode = Enum.OrientationAlignmentMode.OneAttachment
            align2.MaxTorque = 1000
            align2.Responsiveness = 50
            align2.RigidityEnabled = true
        end

        local targetHRP, targetChar = getBestAimbotTarget(hrp2)
        
        if not targetHRP then
            _aimbotLockedTarget = nil
            hrp2.AssemblyLinearVelocity = Vector3.new(0, 0, 0)
            return
        end

        local targetVel = targetHRP.AssemblyLinearVelocity
        local predictTime = math.clamp(targetVel.Magnitude / 120, 0.05, 0.18)
        local predictedPos = targetHRP.Position + (targetVel * predictTime)

        local aimTarget = predictedPos + Vector3.new(0, BatAimbot.VERTICAL_OFFSET, 0)

        align2.CFrame = CFrame.lookAt(hrp2.Position, aimTarget)

        local dir = (aimTarget - hrp2.Position)
        local standPos = aimTarget - (dir.Unit * BatAimbot.BEHIND_DISTANCE)
        standPos += Vector3.new(0, BatAimbot.HEIGHT_OFFSET, 0)

        local moveDir = (standPos - hrp2.Position)
        
        if moveDir.Magnitude > 0.1 then
            maintainConstantSpeed(hrp2, moveDir)
        else
            maintainConstantSpeed(hrp2, hrp2.CFrame.LookVector)
        end
        
        hrp2.AssemblyLinearVelocity = hrp2.AssemblyLinearVelocity
    end)
end

local function stopBatAimbotInternal()
    if not BatAimbot.batAimbotEnabled then return end
    BatAimbot.batAimbotEnabled = false

    if BatAimbot.aimbotConn then 
        BatAimbot.aimbotConn:Disconnect() 
        BatAimbot.aimbotConn = nil
    end
    ...
    if char then
        local hrp = char:FindFirstChild("HumanoidRootPart")
        if hrp then
            local att = hrp:FindFirstChild("AimbotAttachment")
            local al = hrp:FindFirstChild("AimbotAlign")
...
BatAimbot.startFn = startBatAimbotInternal
BatAimbot.stopFn = stopBatAimbotInternal

local function enableBatAimbot()
    if BatAimbot.Enabled then return end
    BatAimbot.Enabled = true
    
    if lp.Character then 
        startBatAimbotInternal() 
    end
...
local function disableBatAimbot()
    ...
    stopBatAimbotInternal()
end

local function createAutoBatGUI()
    if BatAimbot.gui then return end

    local playerGui = lp:WaitForChild("PlayerGui", 10)
    if not playerGui then return end

    local screenGui = Instance.new("ScreenGui")
    screenGui.Name = "AutoBatGUI"
    screenGui.ResetOnSpawn = false
    screenGui.Parent = playerGui
    BatAimbot.gui = screenGui
    ...
    BatAimbot.btn = toggleBtn

    local function makeInput(labelText, default, yPos, callback)
    ...
    makeInput("Speed:", BatAimbot.SPEED, 110, function(v) BatAimbot.SPEED = v end)
    makeInput("Behind Distance:", BatAimbot.BEHIND_DISTANCE, 145, function(v) BatAimbot.BEHIND_DISTANCE = v end)
    makeInput("Height Offset:", BatAimbot.HEIGHT_OFFSET, 180, function(v) BatAimbot.HEIGHT_OFFSET = v end)
    makeInput("Vertical Offset:", BatAimbot.VERTICAL_OFFSET, 215, function(v) BatAimbot.VERTICAL_OFFSET = v end)
    ...
    toggleBtn.MouseButton1Click:Connect(function()
        if BatAimbot.Enabled then
            ...
            statusLabel.Text = "Status: Running (58 Constant)"
            statusLabel.TextColor3 = Color3.fromRGB(100, 255, 100)
            enableBatAimbot()
            if lp.Character then startBatAimbotInternal() end
        end
    end)
...
local C_RED_ACC = Color3.fromRGB(75,0,130)      -- replace red with indigo
	...
	do
		...
		mkLbl(row,"TP Down")
		local btn=Instance.new("TextButton",row);btn.Size=UDim2.new(0,72,0,24);btn.Position=UDim2.new(1,-82,0.5,-12)
		btn.BackgroundTransparency=1;btn.BorderSizePixel=0;btn.Text=(KB.TPFloor.kb or KB.TPFloor.gp or Enum.KeyCode.Unknown).Name;btn.TextColor3=C_WHITE;btn.Font=Enum.Font.GothamBold;btn.TextSize=10;btn.ZIndex=6
